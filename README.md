...
LangSmith documentation tends to use OpenAI + a single chain. This repo demonstrates a more realistic setup: a **multi-agent graph**, **fully local models**, and **all three evaluator categories** working together on a use case that actually matters.

A **multi-agent compound AI system** that detects greenwashing in text and decides for every flagged phrase whether to **keep**, **replace**, or **delete** it. Runs entirely on **small offline models** (Ollama / HuggingFace) and is **fully traced and evaluated with LangSmith**.
...
Each agent is a LangGraph node. Routing is conditional on detector output — if nothing is flagged, the graph s skips the downstream agents. 
Every node is decorated with `@traceable`, so the full call tree appears in LangSmith with per-node latency, tokens, and inputs/outputs.
...
```bash
git clone https://github.com/<you>/greenwashing-detector.git
...
```bash
# CLI
greenwash analyse "Our packaging is 100% eco-friendly and helps save the planet."

# or from a file
greenwash analyse --file my_document.txt --out result.json
